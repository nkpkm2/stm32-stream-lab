...
set $i = 0
while $i < 10
  printf "ct_trace_%u_request_id=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].request_id
  printf "ct_trace_%u_payload_length=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].payload_length
  printf "ct_trace_%u_rx_complete_cycle=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].rx_complete_cycle
  printf "ct_trace_%u_processed_cycle=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].processed_cycle
  printf "ct_trace_%u_reply_start_cycle=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].reply_start_cycle
  printf "ct_trace_%u_reply_complete_cycle=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].reply_complete_cycle
  printf "ct_trace_%u_w6_input=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].w6_input_at_process
  printf "ct_trace_%u_w6_phase=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].w6_phase_at_process
  printf "ct_trace_%u_payload_crc32=0x%08x\n", $i, (unsigned int)g_r2_ct_result.trace[$i].payload_crc32
  printf "ct_trace_%u_processed_while_running=%u\n", $i, (unsigned int)g_r2_ct_result.trace[$i].processed_while_running
  set $i = $i + 1
end
