#!/usr/bin/env python3
"""Offline acceptance for CT-W3 Attempt 02.
...
BUILD = Path(
    r"E:\Projects\stm32-stream-lab\build\r2-ct-w3-k8-normal-800-t1500-48da792e"
)

CHECKPOINT = "48da792e382e911aad1b7bb43765284aa8b58ea2"
EXPECTED_ELF = "9803E7A3974C08913C18B8285807971253680B9ACAA64572F2BCECBC4A0E578A"
EXPECTED_BIN = "D83E2B181F33EB1A5D994D61DA02083D096C23181015A8795E378152DC9C44F7"
EXPECTED_HOST = "C91EE670D0B7B5395F479A6238EA87D31DF4708584E4240551732832C341C29D"

ELF = BUILD / "cubemx.elf"
BIN = BUILD / "cubemx.bin"
HOST_TOOL = REPO / r"tools\r2\r2_ct_uniform.py"

H2_LOG = BUILD / "hardware-attempt02-flash-verify.txt"
H3_SUMMARY = BUILD / "hardware-attempt02-h3-runtime.json"
H3_STDOUT = BUILD / "hardware-attempt02-uniform.stdout.txt"
H3_STDERR = BUILD / "hardware-attempt02-uniform.stderr.txt"
...
def parse_gdb(path: Path) -> dict[str, int]:
    ...
    for raw in path.read_text(
        ...
        if match:
            values[match.group(1)] = parse_value(
                match.group(2)
            )
...
def main() -> int:
    print("=== CT-W3 ATTEMPT 02 / H5 OFFLINE ACCEPTANCE ===")
    ...
    for path in (OUT_JSON, OUT_TXT):
        if path.exists():
            raise RuntimeError(
                f"refusing to overwrite H5 result: {path}"
            )
    ...
    if head != CHECKPOINT or origin != CHECKPOINT:
        raise AssertionError(
            f"wrong checkpoint: HEAD={head}, origin={origin}"
        )
    ...
    if elf_hash != EXPECTED_ELF:
        raise AssertionError(
            f"ELF SHA256 mismatch: {elf_hash}"
        )

    if bin_hash != EXPECTED_BIN:
        raise AssertionError(
            f"BIN SHA256 mismatch: {bin_hash}"
        )

    if host_hash != EXPECTED_HOST:
        raise AssertionError(
            f"host tool SHA256 mismatch: {host_hash}"
        )

    h2_text = H2_LOG.read_text(
        encoding="utf-8",
        errors="replace",
    )
    ...
    if "Download verified successfully" not in h2_text:
        raise AssertionError(
            "Attempt-02 H2 flash/verify success marker missing"
        )

    print("Committed checkpoint/artifact identity: PASS")
    print("Attempt-02 H2 flash/verify evidence: PASS")
    ...
    print()
    print("=== 2. ATTEMPT-02 H3 HOST-RUNTIME EVIDENCE ===")

    h3 = json.loads(
        H3_SUMMARY.read_text(encoding="utf-8")
    )
    ...
    stderr_text = H3_STDERR.read_text(
        encoding="utf-8",
        errors="replace",
    ).strip()
    ...
    if stderr_text:
        raise AssertionError(
            f"H3 host stderr is non-empty: {stderr_text}"
        )

    if h3.get("result") != "PASS":
        raise AssertionError(
            "H3 summary does not report PASS"
        )

    if h3.get("attempt") != 2:
        raise AssertionError(
            f"H3 attempt marker is not 2: {h3.get('attempt')}"
        )

    if raw_host.get("result") != "PASS":
        raise AssertionError(
            "raw host JSON does not report PASS"
        )

    if h3.get("profile") != "CT-W3_UNIFORM_10HZ":
        raise AssertionError(
            f"unexpected H3 profile: {h3.get('profile')!r}"
        )

    if raw_host.get("profile") != "CT-W3_UNIFORM_10HZ":
        raise AssertionError(
            f"unexpected raw host profile: "
            f"{raw_host.get('profile')!r}"
        )

    if (
        h3.get("request_count") != 10
        or raw_host.get("request_count") != 10
    ):
        raise AssertionError(
            "H3 did not complete exactly 10 requests"
        )

    if (
        h3.get("payload_bytes_each") != 64
        or raw_host.get("payload_bytes_each") != 64
    ):
        raise AssertionError(
            "H3 payload size is not 64 bytes"
        )

    if h3.get("interval_ms_nominal") != 100.0:
        raise AssertionError(
            "H3 nominal interval is not 100 ms"
        )

    if raw_host.get("interval_ms") != 100.0:
        raise AssertionError(
            "raw host interval is not 100 ms"
        )

    if h3.get("debugger_attached") is not False:
        raise AssertionError(
            "debugger was attached during H3"
        )

    if h3.get("flash_performed") is not False:
        raise AssertionError(
            "H3 unexpectedly performed flash"
        )

    if h3.get("host_returncode") != 0:
        raise AssertionError(
            "H3 host return code is not zero"
        )

    span_ms = float(
        h3.get("actual_start_span_ms")
    )

    if not (0.0 < span_ms < 1000.0):
        raise AssertionError(
            "10 request starts did not fit inside "
            f"one 1000 ms window: {span_ms} ms"
        )
    ...
    if not isinstance(records, list) or len(records) != 10:
        raise AssertionError(
            "H3 summary records must contain exactly 10 entries"
        )

    if records != raw_records:
        raise AssertionError(
            "H3 summary records differ from raw host records"
        )
    ...
    if actual_ids != expected_ids:
        raise AssertionError(
            f"request ID sequence mismatch: {actual_ids}"
        )
    ...
    if any(
        ...
        raise AssertionError(
            "host W6 inputs are not strictly increasing: "
            f"{host_inputs}"
        )

    if int(h3.get("first_w6_input")) != host_inputs[0]:
        raise AssertionError(
            "H3 first_w6_input mismatch"
        )

    if int(h3.get("last_w6_input")) != host_inputs[-1]:
        raise AssertionError(
            "H3 last_w6_input mismatch"
        )

    for index, record in enumerate(records):
        if int(record["target_phase"]) != 3:
            raise AssertionError(
                f"host record {index} phase != RUNNING"
            )

        if int(record["target_k"]) != 8:
            raise AssertionError(
                f"host record {index} K != 8"
            )

        if int(record["target_drop_mode"]) != 0:
            raise AssertionError(
                f"host record {index} mode != NORMAL"
            )

    print("H3 10/10 uniform requests: PASS")
    print(
        f"Actual first-to-last start span: "
        f"{span_ms:.3f} ms"
    )
    print(
        f"Host W6 input range: "
        f"{host_inputs[0]} -> {host_inputs[-1]}"
    )
    ...
    for index, request_id in enumerate(expected_ids):
        ...
        expect(
            v,
            f"{prefix}_processed_while_running",
            1,
        )

        target_input = require(
            v,
            f"{prefix}_w6_input",
        )
        ...
        expect(
            v,
            f"{prefix}_payload_crc32",
            expected_crc,
        )

        host_crc = int(
            str(records[index]["payload_crc32"]),
            16,
        )
        ...
        rx_cycle = require(
            v,
            f"{prefix}_rx_complete_cycle",
        )

        processed_cycle = require(
            v,
            f"{prefix}_processed_cycle",
        )

        reply_start_cycle = require(
            v,
            f"{prefix}_reply_start_cycle",
        )

        reply_complete_cycle = require(
            v,
            f"{prefix}_reply_complete_cycle",
        )
        ...
        for name, value in (
            ...
            if value == 0:
                raise AssertionError(
                    f"trace {index} {name} cycle is zero"
                )

        d_rx_process = u32_delta(
            rx_cycle,
            processed_cycle,
        )

        d_process_start = u32_delta(
            processed_cycle,
            reply_start_cycle,
        )
        ...
        for delta in (
            ...
            if delta == 0 or delta >= 180_000_000:
                raise AssertionError(
                    f"trace {index} timestamp ordering/"
                    f"sanity failed: "
                    f"{d_rx_process}, "
                    f"{d_process_start}, "
                    f"{d_reply}, "
    ...
    if target_inputs != host_inputs:
        raise AssertionError(
            "target trace W6 inputs differ from host records"
        )

    max_rx_process = max(rx_to_process)
    max_process_reply = max(
        process_to_reply_start
    )
    max_reply_dma = max(reply_dma)
    max_total_service = max(
        rx_to_reply_complete
    )

    print("CT counters/errors: PASS")
    print(
        "CT trace 0..9 request IDs / CRC / "
    ...
    if decision > 57600:
        raise AssertionError(
            f"W6 decision timing failed: "
            f"{decision} > 57600"
        )

    if irq_exit > 80640:
        raise AssertionError(
            f"W6 IRQ-exit timing failed: "
            f"{irq_exit} > 80640"
        )

    if final_window > 3600:
        raise AssertionError(
            f"W6 final-window timing failed: "
            f"{final_window} > 3600"
        )
    ...
    print(
        f"W6 timing: "
        f"{decision} / {irq_exit} / {final_window} cycles"
    )
    print(
        "W6 DMA/ADC/ownership/sample/canary errors: 0"
    )
    print(
    ...
    result = {
        ...
        "attempt": 2,
        "scope": (
            "CT-W3 K8 NORMAL worst-permitted "
            "uniform real control traffic"
        ),
        ...
        "nominal_interval_ms": 100.0,
        "actual_start_span_ms": span_ms,
        "first_w6_input": host_inputs[0],
        "last_w6_input": host_inputs[-1],
        "ct_rx_byte_count": require(
            v,
            "ct_rx_byte_count",
        ),
        "ct_processed_count": require(
            v,
            "ct_processed_count",
        ),
        "ct_reply_success_count": require(
            v,
            "ct_reply_success_count",
        ),
        "ct_rate_limited_count": require(
            v,
            "ct_rate_limited_count",
        ),
        "ct_max_rx_to_process_cycles": (
            max_rx_process
        ),
        "ct_max_process_to_reply_start_cycles": (
            max_process_reply
        ),
        "ct_max_reply_dma_cycles": (
            max_reply_dma
        ),
        "ct_max_rx_to_reply_complete_cycles": (
            max_total_service
        ),
        "w6_input_count": require(
            v,
            "w6_input_count",
        ),
        "w6_admitted_count": require(
            v,
            "w6_admitted_count",
        ),
        "w6_capacity_drop_count": require(
            v,
            "w6_capacity_drop_count",
        ),
        "w6_processed_count": require(
            v,
            "w6_processed_count",
        ),
        "w6_released_count": require(
            v,
            "w6_released_count",
        ),
        "w6_max_nominal_to_decision_cycles": (
        ...
        "evidence_sha256": {
            ...
            H3_SUMMARY.name: sha256(
                H3_SUMMARY
            ),
            H3_STDOUT.name: sha256(
                H3_STDOUT
            ),
            H3_STDERR.name: sha256(
                H3_STDERR
            ),
            H4_INSPECTION.name: sha256(
                H4_INSPECTION
            ),
        },
    }
    ...
    lines = [
        "R2 CT-W3 UNIFORM HARDWARE ATTEMPT 02: PASS",
        "",
        ...
        (
            "Actual first-to-last start span: "
            f"{span_ms:.3f} ms"
        ),
        (
            "W6 input range: "
        ...
        "",
        (
            "CT processed / while-running / "
            "reply-success: 10 / 10 / 10"
        ),
        "CT rate-limited: 0",
        "CT parser/UART/overflow/reply errors: 0",
        (
            "CT max service cycles "
        ...
        "W6 fault_bits: 0",
        (
            "W6 max decision cycles: "
            f"{decision} / 57600"
        ),
        (
            "W6 max IRQ-exit cycles: "
            f"{irq_exit} / 80640"
        ),
        (
            "W6 max final window cycles: "
            f"{final_window} / 3600"
        ),
    ...
    print()
    print("=== CT-W3 ATTEMPT 02 / H5 RESULT ===")
    print("Uniform acceptance: PASS")
    print("Traffic:            10 x 64 B PING")
    print(
        f"Actual start span:  {span_ms:.3f} ms"
    )
    print(
        f"W6 input range:     "
        f"{host_inputs[0]} -> {host_inputs[-1]}"
    ...
    print()
    print(
        f"Acceptance JSON:    {OUT_JSON}"
    )
    print(
        f"Acceptance TXT:     {OUT_TXT}"
    )
    print("Hardware:           NOT ACCESSED")
    print()
    print(
        "CT-W3: CLOSED / KNOWN-GOOD UNIFORM"
    )
    print(
...
if __name__ == "__main__":
    try:
        ...
        print()
        print(
            "=== CT-W3 ATTEMPT 02 / H5 RESULT ==="
        )
        print("Uniform acceptance: FAIL")
        print(str(exc))
        print("Hardware: NOT ACCESSED")
        raise SystemExit(1)
