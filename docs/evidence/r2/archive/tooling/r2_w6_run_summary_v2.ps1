...
Write-Output "=== R2-W6 K=$K $Mode RUN + SUMMARY V2 PREFLIGHT ==="
...
Write-Output "Active buffers (K+2):    $activeCount"
...
$gdbText =
    $gdbTemplate.Replace('__ACTIVE_COUNT__', [string]$activeCount)
...
if states.count(1) != k:
    raise SystemExit(
        f"FAIL: active FREE-state count {states.count(1)} != {k}: {states}"
    )

if states.count(2) != 2:
    raise SystemExit(
        f"FAIL: active DMA_OWNED-state count {states.count(2)} != 2: {states}"
    )

if any(s not in (1, 2) for s in states):
    raise SystemExit(
        f"FAIL: active pool contains unexpected state: {states}"
    )

if mode == "NORMAL":
    if admitted != 96 or drops != 0:
        raise SystemExit(
            f"FAIL: NORMAL admitted={admitted}, drops={drops}"
        )

    if recoveries != 0 or max_drop_streak != 0:
        raise SystemExit(
            f"FAIL: NORMAL recoveries={recoveries}, "
    ...
    if recoveries < 3:
        raise SystemExit(
            f"FAIL: DROP recoveries {recoveries} < 3"
        )

    if max_drop_streak < 3:
        raise SystemExit(
            f"FAIL: DROP max streak {max_drop_streak} < 3"
        )
...
print(f"Active buffer count:         {active_count}")
print("Per-buffer accounting:       PASS")
