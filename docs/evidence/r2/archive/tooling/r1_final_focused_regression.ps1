...
$gdbScriptA = Join-Path $build 'r1-final-regression-a.gdb'
$gdbScriptB = Join-Path $build 'r1-final-regression-b.gdb'
$gdbOutA = Join-Path $build 'r1-final-regression-a.txt'
$gdbOutB = Join-Path $build 'r1-final-regression-b.txt'
$gdbErrA = Join-Path $build 'r1-final-regression-a.stderr.txt'
$gdbErrB = Join-Path $build 'r1-final-regression-b.stderr.txt'

$serverOut = Join-Path $build 'r1-final-regression-gdbserver.stdout.txt'
$serverErr = Join-Path $build 'r1-final-regression-gdbserver.stderr.txt'
$vcpCapturePath = Join-Path $build 'r1-final-regression-vcp.txt'
$summaryPath = Join-Path $build 'r1-final-focused-regression.txt'
...
Write-Output '=== R1 FINAL FOCUSED HARDWARE REGRESSION ==='
...
try {
    ...
    # -------------------------------------------------------------------------
    # 3. Detect ST-LINK Virtual COM port
    # -------------------------------------------------------------------------
    Write-Output ''
    ...
    # -------------------------------------------------------------------------
    # 4. Open VCP before reset so the startup banner cannot be missed
    # -------------------------------------------------------------------------
    Write-Output ''
    ...
    # -------------------------------------------------------------------------
    # 5. Flash / verify / reset exact committed-state ELF
    # -------------------------------------------------------------------------
    Write-Output ''
    Write-Output '=== 5. Flash exact committed-state ELF ==='

    $flashOut = Join-Path $build 'r1-final-regression-flash.stdout.txt'
    $flashErr = Join-Path $build 'r1-final-regression-flash.stderr.txt'
    ...
    [System.IO.File]::WriteAllLines($gdbScriptA, $commonA, $enc)
    [System.IO.File]::WriteAllLines($gdbScriptB, $commonB, $enc)

    Write-Output 'Debugger snapshot scripts: created'

    # -------------------------------------------------------------------------
    # 7. Start attach-mode GDB server
    # -------------------------------------------------------------------------
    Write-Output ''
    Write-Output '=== 7. Start attach-mode debugger ==='
    ...
    # -------------------------------------------------------------------------
    # 8. Snapshot A, resume, wait, snapshot B
    # -------------------------------------------------------------------------
    Write-Output ''
    Write-Output '=== 8. Runtime snapshots ==='

    $gdbProcA = Start-Process `
        -FilePath $gdb `
        -ArgumentList @('-q', '-batch', '-x', $gdbScriptA, $elf) `
        -Wait `
        -PassThru `
        -NoNewWindow `
        -RedirectStandardOutput $gdbOutA `
        -RedirectStandardError $gdbErrA

    $gdbProcA.Refresh()

    if ($gdbProcA.ExitCode -ne 0) {
        if (Test-Path -LiteralPath $gdbOutA) {
            Get-Content $gdbOutA | Select-Object -Last 100
        }

        if (Test-Path -LiteralPath $gdbErrA) {
            Get-Content $gdbErrA | Select-Object -Last 100
        }

        throw 'Focused debugger snapshot A failed.'
    }

    Start-Sleep -Milliseconds 250

    $gdbProcB = Start-Process `
        -FilePath $gdb `
        -ArgumentList @('-q', '-batch', '-x', $gdbScriptB, $elf) `
        -Wait `
        -PassThru `
        -NoNewWindow `
        -RedirectStandardOutput $gdbOutB `
        -RedirectStandardError $gdbErrB

    $gdbProcB.Refresh()

    if ($gdbProcB.ExitCode -ne 0) {
        if (Test-Path -LiteralPath $gdbOutB) {
            Get-Content $gdbOutB | Select-Object -Last 100
        }

        if (Test-Path -LiteralPath $gdbErrB) {
            Get-Content $gdbErrB | Select-Object -Last 100
        }

        throw 'Focused debugger snapshot B failed.'
    }
    ...
    # -------------------------------------------------------------------------
    # 10. R0 regression classification
    # -------------------------------------------------------------------------
    Write-Output ''
    ...
    if ($uwTickDelta -lt 100 -or $uwTickDelta -gt 2000) {
        throw "HAL tick did not advance correctly: delta=$uwTickDelta"
    }

    if ($xTickDelta -lt 100 -or $xTickDelta -gt 2000) {
    ...
    Write-Output 'SVC priority: 0'
    Write-Output 'PendSV priority: 15'
    Write-Output 'SysTick priority: 15'
    Write-Output 'TIM7 priority: 0'
    Write-Output 'DMA2 Stream0 priority: 5'
    Write-Output 'VCP startup banner: PASS'

    # -------------------------------------------------------------------------
    # 11. R1 live acquisition sanity classification
    # -------------------------------------------------------------------------
    Write-Output ''
    ...
    Write-Output 'TIM2 trigger: 200 kHz configuration PASS'
    ...
    Write-Output ''
    Write-Output '============================================================'
    Write-Output 'R1 FINAL FOCUSED HARDWARE REGRESSION: PASS'
    Write-Output '============================================================'
    ...
    Write-Output ''
    Write-Output '============================================================'
    Write-Output 'R1 FINAL FOCUSED HARDWARE REGRESSION: FAIL'
    Write-Output '============================================================'
