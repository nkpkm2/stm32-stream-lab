...
[CmdletBinding()]
param(
    [ValidateSet('Native','W6Target','W5Regression','W4Regression','W3Regression','R1Baseline')]
...
try {
    ...
    $profileLabel = if ($Profile -eq 'W6Target') {
        ('{0}-K{1}-{2}' -f $Profile, $K, $modeUpper)
    } else {
        $Profile
    }

    Write-Host ('=== R2-W6 VERIFICATION: {0} ===' -f $profileLabel)
    ...
    $buildSuffix = if ($Profile -eq 'W6Target') {
        ('w6target-k{0}-{1}' -f $K, $modeUpper.ToLowerInvariant())
    } else {
        $Profile.ToLowerInvariant()
    }

    $build = Join-Path $Repo (
        'build\r2-w6-' + $buildSuffix + '-' +
    ...
    if ($Profile -eq 'Native') {
        ...
        if ($reuseCurrentMsvc) {
            ...
            Write-Host 'MSVC environment: initialized with VsDevCmd.bat'
        }
        ...
        $w4 = if ($Profile -eq 'W4Regression') { 'ON' } else { 'OFF' }
        $w5 = if ($Profile -eq 'W5Regression') { 'ON' } else { 'OFF' }
        $w6 = if ($Profile -eq 'W6Target') { 'ON' } else { 'OFF' }
        $config = @('-S',$source,'-B',$build,'-G','Ninja','-DCMAKE_BUILD_TYPE=Debug',
            ('-DCMAKE_MAKE_PROGRAM=' + $Ninja),
            ('-DCMAKE_TOOLCHAIN_FILE=' + (Join-Path $source 'cmake\gcc-arm-none-eabi.cmake')),
            ('-DSTREAM_LAB_R2_W3=' + $w3),
            ('-DSTREAM_LAB_R2_W4=' + $w4),
            ('-DSTREAM_LAB_R2_W5=' + $w5),
            ('-DSTREAM_LAB_R2_W6=' + $w6),
            ('-DSTREAM_LAB_R2_W6_K=' + $K),
            ('-DSTREAM_LAB_R2_W6_MODE=' + $modeUpper))
    }
    ...
    if ($Profile -eq 'Native') {
        ...
        if (@($listing.tests).Count -ne 116) {
            throw 'Expected exactly 116 registered W1/W2/W3/W4/W5/W6 tests.'
        }
        ...
        foreach ($name in @(
            'test_r2_buffer_pool.exe',
            'test_r2_dma_slots.exe',
            'test_r2_w3_guard.exe',
            'test_r2_w4_model.exe',
            'test_r2_w5_model.exe',
            'test_r2_w6_k1_normal.exe',
            'test_r2_w6_k1_drop.exe',
            'test_r2_w6_k2_normal.exe',
            'test_r2_w6_k2_drop.exe',
            'test_r2_w6_k4_normal.exe',
            'test_r2_w6_k4_drop.exe',
            'test_r2_w6_k8_normal.exe',
            'test_r2_w6_k8_drop.exe')) {
        ...
        if ($Profile -eq 'W6Target') {
            $required = @('R2_W6_CreateTasks','R2_W6_IrqEnter','R2_W6_IrqExit','R2_W6_TraceQueueSend','g_r2_w6_result')
            $forbidden = @('R1_Acquisition_Start','R2_W3_CreateTask','R2_W4_CreateTasks','R2_W5_CreateTasks')
        } elseif ($Profile -eq 'W5Regression') {
            $required = @('R2_W5_CreateTasks','R2_W5_IrqEnter','R2_W5_IrqExit','R2_W5_TraceQueueSend','g_r2_w5_result')
            $forbidden = @('R1_Acquisition_Start','R2_W3_CreateTask','R2_W4_CreateTasks','R2_W6_CreateTasks')
        } elseif ($Profile -eq 'W4Regression') {
            $required = @('R2_W4_CreateTasks','R2_W4_IrqEnter','R2_W4_IrqExit','R2_W4_TraceQueueSend','g_r2_w4_result')
            $forbidden = @('R1_Acquisition_Start','R2_W3_CreateTask','R2_W5_CreateTasks','R2_W6_CreateTasks')
        } elseif ($Profile -eq 'W3Regression') {
            $required = @('R2_W3_CreateTask','R2_W3_IrqEnter','R2_W3_IrqExit','g_r2_w3_result')
            $forbidden = @('R1_Acquisition_Start','R2_W4_CreateTasks','R2_W5_CreateTasks','R2_W6_CreateTasks')
        } else {
            $required = @('R1_Acquisition_Start','R1_Bringup_CreateTask','g_r1_soak_result')
            $forbidden = @('R2_W3_CreateTask','R2_W4_CreateTasks','R2_W5_CreateTasks','R2_W6_CreateTasks')
        }
    ...
    if (@(Git-Lines -Arguments @('rev-parse','HEAD'))[0] -ne $head) {
        throw 'HEAD changed.'
    }
    ...
    if ($Profile -eq 'Native') {
        $report += 'W1/W2/W3/W4/W5/W6 native tests: 116 / 116 PASS'
    }
    if ($Profile -eq 'W6Target') {
        $report += ('W6 K: {0}' -f $K)
        $report += ('W6 mode: {0}' -f $modeUpper)
    }
    ...
    Write-Host ''
    Write-Host 'R2-W6 VERIFICATION: PASS'
    $report | ForEach-Object { Write-Host $_ }
    Write-Host 'No flash, reset, commit, push, or gate-tag operation.'
} catch {
    Write-Host ''
    Write-Host 'R2-W6 VERIFICATION: FAIL'
