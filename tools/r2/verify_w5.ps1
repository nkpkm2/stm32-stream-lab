...
[CmdletBinding()]
param(
    [ValidateSet('Native','W5Target','W4Regression','W3Regression','R1Baseline')]
...
try {
    Write-Host ('=== R2-W5 VERIFICATION: {0} ===' -f $Profile)
    ...
    $build = Join-Path $Repo ('build\r2-w5-' + $Profile.ToLowerInvariant() + '-' + [guid]::NewGuid().ToString('N').Substring(0,8))
    ...
    if ($Profile -eq 'Native') {
        ...
        $w5 = if ($Profile -eq 'W5Target') { 'ON' } else { 'OFF' }
        $config = @('-S',$source,'-B',$build,'-G','Ninja','-DCMAKE_BUILD_TYPE=Debug',
    ...
    if ($Profile -eq 'Native') {
        ...
        if (@($listing.tests).Count -ne 84) { throw 'Expected exactly 84 registered W1/W2/W3/W4/W5 tests.' }
        ...
        foreach ($name in @('test_r2_buffer_pool.exe','test_r2_dma_slots.exe','test_r2_w3_guard.exe','test_r2_w4_model.exe','test_r2_w5_model.exe')) {
        ...
        if ($Profile -eq 'W5Target') {
            ...
            $forbidden = @('R1_Acquisition_Start','R2_W3_CreateTask','R2_W5_CreateTasks')
        } elseif ($Profile -eq 'W3Regression') {
            $required = @('R2_W3_CreateTask','R2_W3_IrqEnter','R2_W3_IrqExit','g_r2_w3_result')
            $forbidden = @('R1_Acquisition_Start','R2_W4_CreateTasks','R2_W5_CreateTasks')
        } else {
            $required = @('R1_Acquisition_Start','R1_Bringup_CreateTask','g_r1_soak_result')
            $forbidden = @('R2_W3_CreateTask','R2_W4_CreateTasks','R2_W5_CreateTasks')
        }
