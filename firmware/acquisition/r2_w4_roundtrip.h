#ifndef R2_W4_ROUNDTRIP_H
#define R2_W4_ROUNDTRIP_H
...
typedef enum
{
    R2_W4_FAULT_CONFIG = 1U << 0,
    R2_W4_FAULT_START = 1U << 1,
    R2_W4_FAULT_EVENT = 1U << 2,
    R2_W4_FAULT_DMA = 1U << 3,
    R2_W4_FAULT_OVR = 1U << 4,
    R2_W4_FAULT_MODEL = 1U << 5,
    R2_W4_FAULT_ADDRESS = 1U << 6,
    R2_W4_FAULT_PRECHECK = 1U << 7,
    R2_W4_FAULT_TIMING = 1U << 8,
    R2_W4_FAULT_FREE_EMPTY = 1U << 9,
    R2_W4_FAULT_READY_FULL = 1U << 10,
    R2_W4_FAULT_PROCESSING = 1U << 11,
    R2_W4_FAULT_RELEASE = 1U << 12,
    R2_W4_FAULT_QUEUE_SOURCE = 1U << 13,
    R2_W4_FAULT_TOKEN_LEDGER = 1U << 14,
    R2_W4_FAULT_STOP = 1U << 15,
    R2_W4_FAULT_TIMEOUT = 1U << 16,
    R2_W4_FAULT_SAMPLES = 1U << 17
} R2_W4_Fault;
...
typedef struct
{
    ...
    uint32_t nominal_to_commit_cycles;
...
typedef struct
{
    ...
    uint32_t buffer_address[R2_W4_BUFFERS];
    uint32_t admitted_by_buffer[R2_W4_BUFFERS];
    uint32_t processed_by_buffer[R2_W4_BUFFERS];
    uint32_t released_by_buffer[R2_W4_BUFFERS];
    R2_BufferPoolSnapshot pool_at_stop;
    R2_DmaSlotsSnapshot slots_at_stop;
    R2_W4_Trace trace[R2_W4_EVENTS];
} R2_W4_Result;

extern volatile R2_W4_Result g_r2_w4_result;

void R2_W4_CreateTasks(void);
void R2_W4_IrqEnter(uint32_t dma_lisr);
void R2_W4_IrqExit(void);

/* Called only through FreeRTOS V11.1.0 traceQUEUE_SEND when W4 is enabled. */
