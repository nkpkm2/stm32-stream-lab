...
#define W5_MAGIC 0x52325735U
#define W5_CONTROL_STACK_WORDS 768U
#define W5_PROCESS_STACK_WORDS 768U
#define W5_DMA_ERRORS (DMA_LISR_TEIF0 | DMA_LISR_DMEIF0 | DMA_LISR_FEIF0)
#define W5_ALL_FLAGS (W5_DMA_ERRORS | DMA_LISR_TCIF0 | DMA_LISR_HTIF0)
#define W5_GUARD_LO 0x13579BDFU
#define W5_GUARD_HI 0x2468ACE0U
#define W5_SENTINEL 0xA55AU
#define W5_RUN_TIMEOUT_MS 1000U
#define W5_DRAIN_TIMEOUT_MS 500U
#define W5_SETTLE_CYCLES 1200U
#define W5_DMA_REQUIRED_CR 0x00062D17U
#define W5_ADC_REQUIRED_CR2 0x16000701U
#define W5_WORK_BIT (1UL << 0)
#define W5_ALL_BUFFER_MASK ((1UL << R2_W5_BUFFERS) - 1UL)

typedef struct
{
    volatile uint32_t guard_lo;
    volatile uint16_t samples[R2_W5_BLOCK_SAMPLES];
...
static W5_Buffer buffers[R2_W5_BUFFERS] __attribute__((aligned(4)));
static StaticTask_t control_task_control;
static StaticTask_t processing_task_control;
static StackType_t control_task_stack[W5_CONTROL_STACK_WORDS];
static StackType_t processing_task_stack[W5_PROCESS_STACK_WORDS];
...
static uint8_t free_queue_storage[R2_W5_K * sizeof(R2_BufferId)];
static uint8_t ready_queue_storage[R2_W5_K * sizeof(W5_BlockDescriptor)];
...
static W5_FreeSendContext free_send_context;

volatile R2_W5_Result g_r2_w5_result;
...
{
    g_r2_w5_result.fault_bits |= bits;
    g_r2_w5_result.test_pass = 0U;
    if (g_r2_w5_result.phase != R2_W5_COMPLETE)
    {
        g_r2_w5_result.phase = R2_W5_QUIESCING;
    }
...
{
    uint32_t errors = DMA2->LISR & W5_DMA_ERRORS;
    uint32_t ovr = ADC1->SR & ADC_SR_OVR;

    g_r2_w5_result.dma_error_flags_seen |= errors;
    g_r2_w5_result.adc_ovr_seen |= ovr;

    return (errors == 0U) &&
        (ovr == 0U) &&
        ((DMA2_Stream0->CR & ~DMA_SxCR_CT) == W5_DMA_REQUIRED_CR) &&
        ((TIM2->CR1 & TIM_CR1_CEN) != 0U) &&
        (ADC1->CR2 == W5_ADC_REQUIRED_CR2);
}

static void LedgerFault(void)
{
    ++g_r2_w5_result.token_ledger_errors;
    LatchFault(R2_W5_FAULT_TOKEN_LEDGER);
}
...
void R2_W5_TraceQueueSend(void *queue_handle)
{
    ...
    {
        ++g_r2_w5_result.illegal_free_send_count;
        LatchFault(R2_W5_FAULT_QUEUE_SOURCE);
        return;
    }

    id = free_send_context.buffer_id;
    if ((uint32_t)id >= R2_W5_BUFFERS)
    {
        ++g_r2_w5_result.illegal_free_send_count;
        LatchFault(R2_W5_FAULT_QUEUE_SOURCE);
        return;
    ...
    if (free_send_context.source == W5_FREE_SEND_INIT)
    {
        ...
        free_token_mask |= bit;
        ++g_r2_w5_result.init_hook_count;
    }
    else if (free_send_context.source == W5_FREE_SEND_COMPLETE)
    {
        if ((R2_BufferPool_GetState(id, &state) != R2_BUFFER_POOL_OK) ||
        ...
        {
            ++g_r2_w5_result.illegal_free_send_count;
            LatchFault(R2_W5_FAULT_RELEASE);
            return;
        }
        free_token_mask |= bit;
        ++g_r2_w5_result.completion_hook_count;
        if ((free_send_context.sequence >= 1U) &&
            (free_send_context.sequence <= R2_W5_EVENTS))
        {
            g_r2_w5_result.trace[free_send_context.sequence - 1U]
                .release_commit_offset_cycles =
                DWT->CYCCNT - g_r2_w5_result.epoch_before;
        }
    }
    else
    {
    ...
    uint32_t sequence)
{
    ...
    {
        if (source == W5_FREE_SEND_COMPLETE)
        {
            LatchFault(R2_W5_FAULT_RELEASE);
        }
        else
    ...
    free_send_context.active = 0U;
    free_send_context.source = W5_FREE_SEND_NONE;
...
void R2_W5_IrqEnter(uint32_t dma_lisr)
{
    ...
    irq_flags = dma_lisr;
    irq_ct = CurrentCt();
    trace_for_exit = R2_W5_EVENTS;
    irq_should_yield = pdFALSE;
    ++g_r2_w5_result.irq_count;

    if (g_r2_w5_result.phase != R2_W5_RUNNING)
    {
        DisableDmaInterrupts();
        return;
    }

    g_r2_w5_result.dma_error_flags_seen |= dma_lisr & W5_DMA_ERRORS;
    g_r2_w5_result.adc_ovr_seen |= ADC1->SR & ADC_SR_OVR;

    if ((dma_lisr & W5_DMA_ERRORS) != 0U)
    {
        LatchFault(R2_W5_FAULT_DMA);
    }
    else if ((ADC1->SR & ADC_SR_OVR) != 0U)
    {
        LatchFault(R2_W5_FAULT_OVR);
    }
    else if ((dma_lisr & DMA_LISR_TCIF0) == 0U)
    {
        LatchFault(R2_W5_FAULT_EVENT);
    }
}
...
static void Completed(DMA_HandleTypeDef *dma, uint32_t completed_slot)
{
    ...
    W5_BlockDescriptor descriptor;
    volatile uint32_t *inactive_address_register;
    volatile uint32_t *active_address_register;
    volatile R2_W5_Trace *trace;

    if ((dma != &hdma_adc1) ||
        (g_r2_w5_result.phase != R2_W5_RUNNING))
    {
        LatchFault(R2_W5_FAULT_EVENT);
        return;
    }

    sequence = g_r2_w5_result.input_count + 1U;
    if ((sequence > R2_W5_EVENTS) ||
    ...
    trace = &g_r2_w5_result.trace[trace_for_exit];
    ...
    {
        LatchFault(R2_W5_FAULT_MODEL);
        return;
    }

    trace->completed_id = observation.completed_buffer;

    if (((uint32_t)observation.completed_buffer >= R2_W5_BUFFERS) ||
        ((uint32_t)observation.active_buffer >= R2_W5_BUFFERS) ||
    ...
    if (free_result != pdPASS)
    {
        ...
        trace->decision = R2_W5_DECISION_DROP;
        ...
        {
            LatchFault(R2_W5_FAULT_PRECHECK);
            __set_PRIMASK(primask);
            return;
        }

        now = DWT->CYCCNT;
        elapsed = now - g_r2_w5_result.epoch_before;
        if (R2_W5_CheckWindow(sequence, irq_ct, CurrentCt(), elapsed,
                trace->ndtr_guard, healthy) != R2_W5_GUARD_OK)
        {
            LatchFault(R2_W5_FAULT_PRECHECK);
            __set_PRIMASK(primask);
            return;
        ...
        trace->nominal_to_decision_cycles =
            now - g_r2_w5_result.epoch_before - nominal;
        ...
        if ((trace->final_window_cycles > R2_W5_FINAL_RESERVE_CYCLES) ||
            (trace->nominal_to_decision_cycles > R2_W5_WRITE_LIMIT_CYCLES))
        {
            LatchFault(R2_W5_FAULT_TIMING);
        }

        __set_PRIMASK(primask);
        if (g_r2_w5_result.fault_bits != 0U)
        {
            return;
        }
        ...
        ++g_r2_w5_result.input_count;
        ++g_r2_w5_result.capacity_drop_count;
        ++g_r2_w5_result.dropped_by_buffer[observation.completed_buffer];
        ++g_r2_w5_result.current_drop_streak;
        if (g_r2_w5_result.current_drop_streak >
            g_r2_w5_result.max_drop_streak)
        {
            g_r2_w5_result.max_drop_streak =
                g_r2_w5_result.current_drop_streak;
        }
        ...
        if (trace->nominal_to_decision_cycles >
            g_r2_w5_result.max_nominal_to_decision_cycles)
        {
            g_r2_w5_result.max_nominal_to_decision_cycles =
                trace->nominal_to_decision_cycles;
        }
        if (trace->final_window_cycles >
            g_r2_w5_result.max_final_window_cycles)
        {
            g_r2_w5_result.max_final_window_cycles =
    ...
    if ((uint32_t)free_depth < g_r2_w5_result.min_free_depth)
    {
        g_r2_w5_result.min_free_depth = (uint32_t)free_depth;
    }
    ...
    trace->decision = R2_W5_DECISION_ADMIT;
    ...
    *inactive_address_register = Address(replacement);
    ...
    if ((*inactive_address_register != Address(replacement)) ||
        ...
        (HardwareHealthy() == 0U))
    {
        LatchFault(R2_W5_FAULT_ADDRESS);
    }
    if ((trace->final_window_cycles > R2_W5_FINAL_RESERVE_CYCLES) ||
        (trace->nominal_to_decision_cycles > R2_W5_WRITE_LIMIT_CYCLES))
    {
    ...
    {
        ++g_r2_w5_result.ready_send_fail_count;
        LatchFault(R2_W5_FAULT_READY_FULL);
        return;
    }
    ...
    if ((uint32_t)ready_depth > g_r2_w5_result.max_ready_depth)
    {
        g_r2_w5_result.max_ready_depth = (uint32_t)ready_depth;
    }

    hpw = pdFALSE;
    if (xTaskNotifyFromISR(processing_task_handle, W5_WORK_BIT,
            eSetBits, &hpw) != pdPASS)
    {
        ++g_r2_w5_result.notification_fail_count;
        LatchFault(R2_W5_FAULT_EVENT);
        return;
    }
    if (hpw != pdFALSE)
    ...
    ++g_r2_w5_result.input_count;
    ++g_r2_w5_result.admitted_count;
    ++g_r2_w5_result.admitted_by_buffer[descriptor.buffer_id];

    if (g_r2_w5_result.current_drop_streak != 0U)
    {
        ++g_r2_w5_result.recovered_admission_after_drop_count;
        g_r2_w5_result.current_drop_streak = 0U;
    }
...
void R2_W5_IrqExit(void)
{
    ...
    uint32_t offset = now - g_r2_w5_result.epoch_before;

    if (trace_for_exit < R2_W5_EVENTS)
    {
        volatile R2_W5_Trace *trace = &g_r2_w5_result.trace[trace_for_exit];
        uint32_t nominal = trace->sequence * R2_W5_BLOCK_CYCLES;
        ...
        else
        {
            ...
            if (trace->nominal_to_irq_exit_cycles > R2_W5_EXIT_LIMIT_CYCLES)
            {
                LatchFault(R2_W5_FAULT_TIMING);
            }
            if (trace->nominal_to_irq_exit_cycles >
                g_r2_w5_result.max_nominal_to_irq_exit_cycles)
            {
                g_r2_w5_result.max_nominal_to_irq_exit_cycles =
...
static int ConfigurationValid(void)
{
    ...
    for (id = 0U; id < R2_W5_BUFFERS; ++id)
    {
        uint32_t address = Address(id);
        if (((address & 3U) != 0U) ||
            (address < 0x20000000U) ||
            (address > 0x20020000U - 2U * R2_W5_BLOCK_SAMPLES))
        {
            return 0;
        }
    }
...
static HAL_StatusTypeDef Start(void)
{
    ...
    g_r2_w5_result.start_dma_cr = DMA2_Stream0->CR;
    g_r2_w5_result.start_dma_ndtr = DMA2_Stream0->NDTR;
    g_r2_w5_result.start_adc_cr2 = ADC1->CR2;
    g_r2_w5_result.start_tim2_cr1 = TIM2->CR1;

    if ((DMA2_Stream0->CR != W5_DMA_REQUIRED_CR) ||
        (DMA2_Stream0->NDTR != R2_W5_BLOCK_SAMPLES) ||
    ...
    g_r2_w5_result.phase = R2_W5_ARMED;
    primask = __get_PRIMASK();
    __disable_irq();
    g_r2_w5_result.phase = R2_W5_RUNNING;
    g_r2_w5_result.epoch_before = DWT->CYCCNT;
    status = HAL_TIM_Base_Start(&htim2);
    __DSB();
    g_r2_w5_result.epoch_after = DWT->CYCCNT;
    __set_PRIMASK(primask);
...
static HAL_StatusTypeDef Stop(void)
{
    ...
    begin = DWT->CYCCNT;
    while ((uint32_t)(DWT->CYCCNT - begin) < W5_SETTLE_CYCLES)
    {
        __NOP();
    }

    g_r2_w5_result.dma_error_flags_seen |= DMA2->LISR & W5_DMA_ERRORS;
    g_r2_w5_result.adc_ovr_seen |= ADC1->SR & ADC_SR_OVR;
...
static void ProcessDescriptor(const W5_BlockDescriptor *descriptor)
{
    ...
    if ((sequence == 0U) ||
        (sequence > R2_W5_EVENTS) ||
        (id >= R2_W5_BUFFERS))
    {
        LatchFault(R2_W5_FAULT_PROCESSING);
        return;
    }

    trace = &g_r2_w5_result.trace[sequence - 1U];
    if ((trace->decision != R2_W5_DECISION_ADMIT) ||
    ...
    /* Deliberately hold the current buffer in PROCESSING long enough to
     * exhaust K=1 and force several consecutive DMA capacity drops. The
     * task remains preemptible by DMA and kernel interrupts. */
    begin = DWT->CYCCNT;
    while ((uint32_t)(DWT->CYCCNT - begin) < R2_W5_PROCESS_HOLD_CYCLES)
    {
        __NOP();
    }

    if ((buffers[id].guard_lo != W5_GUARD_LO) ||
        (buffers[id].guard_hi != W5_GUARD_HI))
    {
        ++g_r2_w5_result.canary_errors;
        LatchFault(R2_W5_FAULT_SAMPLES);
        return;
    }

    for (j = 0U; j < R2_W5_BLOCK_SAMPLES; ++j)
    {
        uint32_t value = buffers[id].samples[j];
        if (value > 4095U)
        {
            ++g_r2_w5_result.sample_errors;
        }
        else
        {
            ++g_r2_w5_result.full_sample_count;
        }
        if (value < min_value)
        {
            min_value = value;
    ...
    if (g_r2_w5_result.sample_errors != 0U)
    {
        LatchFault(R2_W5_FAULT_SAMPLES);
        return;
    }

    if (SendFreeToken(W5_FREE_SEND_COMPLETE,
    ...
    ++g_r2_w5_result.processed_count;
    ++g_r2_w5_result.released_count;
    ++g_r2_w5_result.processed_by_buffer[id];
    ++g_r2_w5_result.released_by_buffer[id];
...
static void ProcessingTask(void *argument)
{
    ...
    for (;;)
    {
        ...
        if ((notification_value & W5_WORK_BIT) != 0U)
        {
...
static void Finalize(void)
{
    ...
    g_r2_w5_result.pool_at_stop = pool;
    g_r2_w5_result.slots_at_stop = slots;
    g_r2_w5_result.free_queue_depth_final =
        (uint32_t)uxQueueMessagesWaiting(free_queue);
    g_r2_w5_result.ready_queue_depth_final =
        (uint32_t)uxQueueMessagesWaiting(ready_queue);
    g_r2_w5_result.free_token_mask_final = free_token_mask;
    g_r2_w5_result.ready_token_mask_final = ready_token_mask;
    ...
    for (id = 0U; id < R2_W5_BUFFERS; ++id)
    {
        ...
        if ((g_r2_w5_result.admitted_by_buffer[id] !=
                g_r2_w5_result.processed_by_buffer[id]) ||
            (g_r2_w5_result.admitted_by_buffer[id] !=
                g_r2_w5_result.released_by_buffer[id]))
        {
            LatchFault(R2_W5_FAULT_PROCESSING);
        }

        dropped_sum += g_r2_w5_result.dropped_by_buffer[id];
    }

    if ((pool.k != R2_W5_K) ||
        (pool.active_count != R2_W5_BUFFERS) ||
        (pool.free_count != R2_W5_K) ||
        (pool.ready_count != 0U) ||
        ...
        (g_r2_w5_result.free_queue_depth_final != R2_W5_K) ||
        (g_r2_w5_result.ready_queue_depth_final != 0U) ||
        (ready_token_mask != 0U) ||
        (free_token_mask != expected_free_mask) ||
        ((free_token_mask | BitFor(slots.m0_buffer) |
          BitFor(slots.m1_buffer)) != W5_ALL_BUFFER_MASK))
    {
        LatchFault(R2_W5_FAULT_MODEL);
    }

    if ((g_r2_w5_result.input_count != R2_W5_EVENTS) ||
        ((g_r2_w5_result.admitted_count +
          g_r2_w5_result.capacity_drop_count) != R2_W5_EVENTS) ||
        (g_r2_w5_result.admitted_count < R2_W5_MIN_ADMISSIONS) ||
        (g_r2_w5_result.capacity_drop_count < R2_W5_MIN_CAPACITY_DROPS) ||
        (g_r2_w5_result.max_drop_streak < R2_W5_MIN_DROP_STREAK) ||
        (g_r2_w5_result.recovered_admission_after_drop_count <
            R2_W5_MIN_RECOVERIES) ||
        (g_r2_w5_result.processed_count != g_r2_w5_result.admitted_count) ||
        (g_r2_w5_result.released_count != g_r2_w5_result.admitted_count) ||
        (g_r2_w5_result.completion_hook_count != g_r2_w5_result.admitted_count) ||
        (g_r2_w5_result.init_hook_count != R2_W5_K) ||
        (g_r2_w5_result.illegal_free_send_count != 0U) ||
        (g_r2_w5_result.ready_send_fail_count != 0U) ||
        (g_r2_w5_result.notification_fail_count != 0U) ||
        (g_r2_w5_result.token_ledger_errors != 0U) ||
        (g_r2_w5_result.dma_error_flags_seen != 0U) ||
        (g_r2_w5_result.adc_ovr_seen != 0U) ||
        (g_r2_w5_result.full_sample_count !=
            g_r2_w5_result.admitted_count * R2_W5_BLOCK_SAMPLES) ||
        (g_r2_w5_result.sample_errors != 0U) ||
        (g_r2_w5_result.canary_errors != 0U) ||
        (dropped_sum != g_r2_w5_result.capacity_drop_count))
    {
        LatchFault(R2_W5_FAULT_EVENT);
    }
}
...
static void ControlTask(void *argument)
{
    ...
    g_r2_w5_result.system_core_clock = SystemCoreClock;
    g_r2_w5_result.aircr = SCB->AIRCR;
    g_r2_w5_result.min_free_depth = R2_W5_K;

    for (id = 0U; id < R2_W5_BUFFERS; ++id)
    {
        buffers[id].guard_lo = W5_GUARD_LO;
        buffers[id].guard_hi = W5_GUARD_HI;
        for (j = 0U; j < R2_W5_BLOCK_SAMPLES; ++j)
        {
            buffers[id].samples[j] = W5_SENTINEL;
        }
        g_r2_w5_result.buffer_address[id] = Address(id);
    }

    if (!ConfigurationValid())
    {
        LatchFault(R2_W5_FAULT_CONFIG);
        g_r2_w5_result.phase = R2_W5_FAILED;
    }
    else
    {
        status = Start();
        g_r2_w5_result.start_status = (uint32_t)status;
        if (status != HAL_OK)
        {
            LatchFault(R2_W5_FAULT_START);
        }

        start_tick = xTaskGetTickCount();
        while (g_r2_w5_result.phase == R2_W5_RUNNING)
        {
            if ((TickType_t)(xTaskGetTickCount() - start_tick) >=
                pdMS_TO_TICKS(W5_RUN_TIMEOUT_MS))
            {
                LatchFault(R2_W5_FAULT_TIMEOUT);
                break;
            }
        ...
        status = Stop();
        g_r2_w5_result.stop_status = (uint32_t)status;
        if ((status != HAL_OK) ||
            ((DMA2_Stream0->CR & DMA_SxCR_EN) != 0U) ||
            ((TIM2->CR1 & TIM_CR1_CEN) != 0U) ||
            ((ADC1->CR2 & (ADC_CR2_ADON | ADC_CR2_DMA)) != 0U))
        {
            LatchFault(R2_W5_FAULT_STOP);
        }

        drain_tick = xTaskGetTickCount();
        while ((g_r2_w5_result.processed_count !=
                g_r2_w5_result.admitted_count) &&
               (g_r2_w5_result.fault_bits == 0U))
        {
            if ((TickType_t)(xTaskGetTickCount() - drain_tick) >=
                pdMS_TO_TICKS(W5_DRAIN_TIMEOUT_MS))
            {
                LatchFault(R2_W5_FAULT_TIMEOUT);
                break;
            }
            (void)xTaskNotify(processing_task_handle,
                W5_WORK_BIT, eSetBits);
            vTaskDelay(pdMS_TO_TICKS(1U));
        }

        if (g_r2_w5_result.fault_bits == 0U)
        {
            uint32_t irq_before = g_r2_w5_result.irq_count;
            uint32_t input_before = g_r2_w5_result.input_count;
            uint32_t admitted_before = g_r2_w5_result.admitted_count;
            uint32_t drop_before = g_r2_w5_result.capacity_drop_count;
            uint32_t processed_before = g_r2_w5_result.processed_count;

            vTaskDelay(pdMS_TO_TICKS(2U));

            g_r2_w5_result.quiet_irq_count = g_r2_w5_result.irq_count;
            g_r2_w5_result.quiet_input_count = g_r2_w5_result.input_count;
            g_r2_w5_result.quiet_admitted_count =
                g_r2_w5_result.admitted_count;
            g_r2_w5_result.quiet_drop_count =
                g_r2_w5_result.capacity_drop_count;
            g_r2_w5_result.quiet_processed_count =
                g_r2_w5_result.processed_count;

            if ((irq_before != g_r2_w5_result.irq_count) ||
                (input_before != g_r2_w5_result.input_count) ||
                (admitted_before != g_r2_w5_result.admitted_count) ||
                (drop_before != g_r2_w5_result.capacity_drop_count) ||
                (processed_before != g_r2_w5_result.processed_count))
            {
                LatchFault(R2_W5_FAULT_EVENT);
            }
        }
        ...
        g_r2_w5_result.test_pass =
            g_r2_w5_result.fault_bits == 0U ? 1U : 0U;
        g_r2_w5_result.phase =
            g_r2_w5_result.test_pass != 0U ?
            R2_W5_COMPLETE : R2_W5_FAILED;
    }
...
void R2_W5_CreateTasks(void)
{
    ...
    g_r2_w5_result.magic = W5_MAGIC;
    g_r2_w5_result.start_status = UINT32_MAX;
    g_r2_w5_result.stop_status = UINT32_MAX;
    g_r2_w5_result.phase = R2_W5_RESET;
    ...
    ready_queue = xQueueCreateStatic(
        R2_W5_K,
        sizeof(W5_BlockDescriptor),
        ready_queue_storage,
        &ready_queue_control);

    if ((free_queue == NULL) || (ready_queue == NULL))
    {
        g_r2_w5_result.fault_bits = R2_W5_FAULT_CONFIG;
        g_r2_w5_result.phase = R2_W5_FAILED;
        return;
    }
    ...
    if ((R2_BufferPool_Activate(R2_W5_K) != R2_BUFFER_POOL_OK) ||
    ...
    for (id = 2U; id < R2_W5_BUFFERS; ++id)
    {
        if (SendFreeToken(W5_FREE_SEND_INIT, (R2_BufferId)id, 0U) != pdPASS)
        {
            g_r2_w5_result.phase = R2_W5_FAILED;
            return;
        }
    ...
    if ((uxQueueMessagesWaiting(free_queue) != R2_W5_K) ||
        (free_token_mask != (W5_ALL_BUFFER_MASK & ~0x3UL)))
    {
        g_r2_w5_result.fault_bits |= R2_W5_FAULT_TOKEN_LEDGER;
        g_r2_w5_result.phase = R2_W5_FAILED;
        return;
    }
    ...
    control_handle = xTaskCreateStatic(
        ControlTask,
        "R2W5Ctl",
        W5_CONTROL_STACK_WORDS,
        NULL,
        tskIDLE_PRIORITY + 2U,
        control_task_stack,
        &control_task_control);

    g_r2_w5_result.processing_task_created =
        processing_task_handle != NULL ? 1U : 0U;
    g_r2_w5_result.control_task_created =
        control_handle != NULL ? 1U : 0U;

    if ((processing_task_handle == NULL) || (control_handle == NULL))
    {
        g_r2_w5_result.fault_bits |= R2_W5_FAULT_START;
        g_r2_w5_result.phase = R2_W5_FAILED;
        return;
    }

    g_r2_w5_result.phase = R2_W5_TASKS_CREATED;
}
